...
class DNAPassportUiTests(unittest.TestCase):
    ...
    def test_passport_visual_renderer_uses_ready_passport_data_only(self) -> None:
        ...
        self.assertNotIn("ФОТО НЕ ДОБАВЛЕНО", source)
        self.assertNotIn("Дата рождения", source)
        self.assertNotIn('"Пол"', source)
        self.assertIn("ОБРАЗЕЦ", source)
        self.assertIn("X хромосома", source)
        self.assertIn("Y хромосома", source)
        self.assertNotIn("Для точного анализа нужны отдельные Y-DNA и mtDNA-тесты", source)
        self.assertNotIn("_draw_overview_lineage", source)
        ...
        self.assertNotIn("расчёт по полному G25-вектору", source)
        self.assertNotIn("итог по G25", source)
        self.assertNotIn("G25-профиль не найден", source)
        self.assertNotIn("полному G25-вектору", source)
